...
var CHARS = [LIGHT_SWITCH_UUID,DIMMER_SETTING_UUID,LIGHT_NAME_PUB_UUID,DISCONNECT_UUID];
...
Robosmart.discoverAll = function(callback,timeout) {
  var devices = [];
  noble.once('stateChange', function() {
    noble.on('discover', function(peripheral) {
      ...
      rs.status = { 
        ...
        on: false, 
        dim: 0,
        name: peripheral.advertisement.localName.replace(/(\w+).+/,"$1")
      } 
      devices.push(rs);
...
Robosmart.prototype.discoverServicesAndCharacteristics = function(callback) {
  this._peripheral.discoverSomeServicesAndCharacteristics([PUBLIC_SERVICE_UUID], CHARS, function(error, services, characteristics) {
